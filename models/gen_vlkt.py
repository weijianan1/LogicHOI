...
from datasets.hico_text_label import hico_text_label, hico_obj_text_label, hico_unseen_index, hico_obj2hoi, hico_verb2hoi
from datasets.vcoco_text_label import vcoco_hoi_text_label, vcoco_obj_text_label, vcoco_obj2hoi, vcoco_verb2hoi 
...
from .ASL import *
from .utils import RelationFeatureExtractor
from .ltn import *
...
class GEN_VLKT(nn.Module):
    def __init__(self, backbone, transformer, vlm, q2l, txt_processors, num_queries, aux_loss=False, args=None):
        super().__init__()
        ...
        clip_label, obj_clip_label, v_linear_proj_weight, hoi_text, obj_text, train_clip_label = \
            self.init_classifier_with_CLIP(vlm, txt_processors, hoi_text_label, obj_text_label, unseen_index)
        num_obj_classes = len(obj_text) - 1  # del nothing
        self.clip_visual_proj = v_linear_proj_weight

        self.q2l = q2l
        self.coarse_relation_extractor = RelationFeatureExtractor(args, in_channels=hidden_dim, out_dim=hidden_dim, num_objs=len(obj_text_label))

        # hidden_dim
        clip_dim = 512
        self.hoi_class_fc = nn.Sequential(
            nn.Linear(clip_dim, args.clip_embed_dim),
    ...
    def init_classifier_with_CLIP(self, vlm, txt_processors, hoi_text_label, obj_text_label, unseen_index):
        device = "cuda" if torch.cuda.is_available() else "cpu"

        text_inputs = [txt_processors["eval"](hoi_text_label[id]) for id in hoi_text_label.keys()]
        ...
        text_inputs_del = [txt_processors["eval"](hoi_text_label[id]) for id in hoi_text_label_del.keys()]
        obj_text_inputs = [txt_processors["eval"](obj_text[1]) for obj_text in obj_text_label]

        with torch.no_grad():
            text_embedding = vlm.extract_features({"text_input": text_inputs})
            text_embedding_del = vlm.extract_features({"text_input": text_inputs_del})
            obj_text_embedding = vlm.extract_features({"text_input": obj_text_inputs})
            v_linear_proj_weight = vlm.visual.proj.detach()

        return text_embedding.float(), obj_text_embedding.float(), v_linear_proj_weight, \
               hoi_text_label_del, obj_text_inputs, text_embedding_del.float()

    def forward(self, samples: NestedTensor, vlm, is_training=True, clip_input=None):
        ...
        with torch.no_grad():
            clip_visual = vlm.extract_features({"image": clip_input})

        tgt = self.input_proj(src)
        inter_hs, h_hs, o_hs, outputs_sub_coord, outputs_obj_coord = self.transformer(tgt, mask,
                                                self.query_embed_h.weight,
                                                self.query_embed_o.weight,
                                                self.pos_guided_embedd.weight,
                                                pos[-1], clip_visual, 
                                                self.hum_bbox_embed, self.obj_bbox_embed, self.coarse_relation_extractor)
        outputs_verb_class, _ = self.q2l(tgt, pos[-1])
        ...
        if self.aux_loss:
            ...
            out['aux_outputs'] = self._set_aux_loss_triplet(outputs_hoi_class, outputs_obj_class, outputs_verb_class,
    ...
    @torch.jit.unused
    def _set_aux_loss_triplet(self, outputs_hoi_class, outputs_obj_class, outputs_verb_class,
        ...
        aux_outputs = {'pred_hoi_logits': outputs_hoi_class[-self.dec_layers: -1],
                       ...
                       'pred_verb_logits': outputs_verb_class[-self.dec_layers: -1]}
...
class SetCriterionHOI(nn.Module):
    ...
    def __init__(self, num_obj_classes, num_queries, num_verb_classes, matcher, weight_dict, eos_coef, losses, args):
        ...
        if self.args.dataset_file == 'hico':
            self.obj2hoi = hico_obj2hoi
            self.verb2hoi = hico_verb2hoi
        elif self.args.dataset_file == 'vcoco':
            self.obj2hoi = vcoco_obj2hoi
            self.verb2hoi = vcoco_verb2hoi

        self.asl_action_loss = AsymmetricLossOptimized(gamma_neg=args.gamma_neg, gamma_pos=args.gamma_pos, clip=0.05, alpha=0.5)
    ...
    def loss_verb_labels(self, outputs, targets, indices, num_interactions):
        ...
        bs, num_queries, _ = outputs['pred_hoi_logits'].size()

        idx = self._get_src_permutation_idx(indices)
        target_classes_o = torch.cat([t['verb_labels'][J] for t, (_, J) in zip(targets, indices)])
        target_classes = torch.zeros((bs, num_queries, src_logits.size(-1))).to(src_logits.device)
        target_classes[idx] = target_classes_o
        target_classes = (target_classes.sum(1) >= 1) * 1.0
    ...
    def mimic_loss(self, outputs, targets, indices, num_interactions):
        ...
        loss_feat_mimic = nn.CosineEmbeddingLoss()(src_feats, target_clip_feats, torch.ones([src_feats.size()[0]]).to(src_feats.device))
        losses = {'loss_feat_mimic': loss_feat_mimic}

        return losses

    def loss_logic_constrains(self, outputs, targets, indices, num_interactions):
        assert 'pred_hoi_logits' in outputs and 'pred_obj_logits' in outputs and 'pred_verb_logits' in outputs
        hoi_logits = outputs['pred_hoi_logits']

        losses = {}
        obj2hoi = torch.tensor(self.obj2hoi).to(hoi_logits.device)
        verb2hoi = torch.tensor(self.verb2hoi).to(hoi_logits.device)
        hoi_logits = hoi_logits.reshape(-1, hoi_logits.shape[-1])
        losses.update(obj2hoi_logic(hoi_logits, obj2hoi, self.args))
        losses.update(verb2hoi_logic(hoi_logits, verb2hoi, self.args))
    ...
    def get_loss(self, loss, outputs, targets, indices, num, **kwargs):
        if 'pred_hoi_logits' in outputs.keys():
            loss_map = {
                ...
                'logic_constraint': self.loss_logic_constrains,
            }
        else:
            loss_map = {
                'obj_labels': self.loss_obj_labels,
...
def build(args):
    ...
    if args.with_clip_label:
        ...
        weight_dict['loss_obj2hoi_me'] = args.hoi_loss_coef * 0.1
        weight_dict['loss_verb2hoi_me'] = args.hoi_loss_coef * 0.1
    ...
    if args.with_distil:
        weight_dict['loss_att_distil'] = args.distil_loss_coef
    ...
    losses = ['hoi_labels', 'verb_labels', 'obj_labels', 'sub_obj_boxes']
    if args.with_mimic:
        losses.append('feats_mimic')
    if args.with_distil:
        losses.append('att_distil')
    if args.with_logic:
        losses.append('logic_constraint')
    ...
    return model, criterion, postprocessors, vlm, vis_processors
