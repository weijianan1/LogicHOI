...
def get_args_parser():
    ...
    parser.add_argument('--lr_clip', default=5e-6, type=float)
    ...
    parser.add_argument('--gamma_pos',default=2.,type=float)
    ...
    parser.add_argument('--no_spatial_relation', dest='use_spatial_relation', action='store_false', default=True)
    parser.add_argument('--spatial_alpha', default=0.8, type=float)

    parser.add_argument('--with_distil', action='store_true', help="Use clip feature mimic")
    parser.add_argument('--distil_loss_coef', default=20, type=float)

    # n_layer
    parser.add_argument('--n_layer', default=0, type=int, help="Number of graph iteration")
    ...
    parser.add_argument('--score_integration', action='store_true', help="Use cql for interaction")

    parser.add_argument('--with_logic', action='store_true', help='')
    parser.add_argument('--me_p', default=2, type=int)
...
def main(args):
    ...
    dataset_train = build_dataset(vis_processors, image_set='train', args=args)
    dataset_val = build_dataset(vis_processors, image_set='val', args=args)
    ...
    if args.eval:
        test_stats = evaluate_hoi(args.dataset_file, model, vlm, postprocessors, data_loader_val,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, vlm, criterion, data_loader_train, optimizer, device, epoch, args.clip_max_norm)
        lr_scheduler.step()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('LogicHOI training and evaluation script', parents=[get_args_parser()])
