...
import numpy as np
hico_obj2hoi = np.zeros((len(hico_obj_text_label), len(hico_text_label)), dtype=np.uint8)
for hoi_ids, (_, obj_ids) in enumerate(hico_text_label.keys()):
    hico_obj2hoi[obj_ids, hoi_ids] = 1

hico_verb2hoi = np.zeros((117, len(hico_text_label)), dtype=np.uint8)
for hoi_ids, (verb_ids, _) in enumerate(hico_text_label.keys()):
    hico_verb2hoi[verb_ids, hoi_ids] = 1
